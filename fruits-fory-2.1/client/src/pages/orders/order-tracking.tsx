...
export default function OrderTrackingView() {
      ...
      try {
        ...
        const transformedOrders: Order[] = clientOrders.map((order: any) => ({
            ...
            items: order.products?.map((product: any, index: number) => ({
                id: `${order.id}-${index}`,
                clientName: order.clientName || 'Client Non Sp√©cifi√©',
                lotName: product.lotName || 'Lot Non Sp√©cifi√©',
      ...
      if (updates.length > 0) {
        const transformedOrders: Order[] = updates.map((order: any) => ({
          ...
          items: order.products?.map((product: any, index: number) => ({
            ...
            lotName: `Lot ${product.name || 'Non Sp√©cifi√©'}`,
            caliber: 'Mixed',
            quantity: product.quantity || 0,
  ...
  // 1) Production lot via sharedLotService (type: 'production')
        ...
        console.log('üóëÔ∏è Creating waste tracking lot for:', lotNumber);
        ...
        console.log('‚úÖ Waste tracking lot created:', wasteTrackingLotId);

        // 5) Create New Entry lot (Multi-lot system)
        console.log('üì¶ Creating new entry lot for:', lotNumber);
        ...
        console.log('‚úÖ New entry lot created:', newEntryLotId);
        ...
        console.log('üéâ All lots created successfully:', { 
          productionLotId, 
          qualitySharedLotId, 
          qcLotId, 
          wasteTrackingLotId, 
                              ...
                              <div key={item.id} className={`flex items-center gap-4 p-4 rounded-lg border ${item.completed ? 'bg-green-50 border-green-200' : 'bg-white border-gray-200 hover:bg-gray-50'}`}>
                                ...
                                <div className={`flex-1 ${item.completed ? 'opacity-60' : ''}`}>
                                  <div className="flex items-center justify-between">
                                    ...
                                    {item.completed && item.completedAt && <span className="text-xs text-green-600">‚úì {new Date(item.completedAt).toLocaleDateString()}</span>}
                                  </div>
                                </div>
                              </div>
                            ))}
              ...
              <div className="p-6 space-y-6">
                      ...
                      <div key={item.id} className="grid grid-cols-1 md:grid-cols-6 gap-4 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div>
                          ...
                          >
                            ...
                            <optgroup label="Shared Lots (Production & Qualit√©)">
                              {sharedLots.map((lot) => (
                                <option key={lot.id} value={lot.lotNumber}>
                                  {lot.lotNumber} ({lot.type === 'production' ? 'Production' : 'Qualit√©'})
                                </option>
                              ))}
                            </optgroup>
                          </select>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-b™Éxä≈0âKﬁl®$CîéHñpã
“R2Là–%6%"d∆$•0˙çB’Ù3¯5]ÕZ»—AeE0Ï#.+O≠™¬Ó˘∑ÆˇÛtzº¸Ü]ıC„ÜÜç›6åõãîê®Ç‘%‚%⁄\Öö(ÏÒz2{kA—ÿ£Î∏˘Ø˜J ïöJ˛„∞&M2)∏t`◊ß·E˜å›˙%Ï\∞¢è)_Uù™}@ﬁÁø˜HË\E6p	G°<Öø3CŒN´^òì’M “ÌÅz?∫›GÑ≠v¬FMâ˙2d¸ÿÇ·Kp…ˆÊiÅÿ‚Ïœïà¯3K‡69#±)”·ûΩgóÎóò≠Y%mjV<Õ.¢û ‘p<SÆùSU∆,;∂ôÎ≠$,qTÌr| Pä«ceπ#˙&*˝h¡˘g‡ÎR<N.#û©ê“sZGáwæG|Â+òQ§„ıÏ1{•
