...
const LotLanding: React.FC = () => {
  ...
  useEffect(() => {
    // If the QR provides a lotNumber, automatically redirect to the multi-lot detail page.
    // We preserve a leading slash in the lot name by encoding it so the receiving page can decode it.
    if (lotNumber) {
      const valueWithLeadingSlash = lotNumber.startsWith('/') ? lotNumber : `/${lotNumber}`;
      const encoded = encodeURIComponent(valueWithLeadingSlash);
      setLocation(`/multi-lot-detail/${encoded}`);
    }
  }, [lotNumber, setLocation]);
